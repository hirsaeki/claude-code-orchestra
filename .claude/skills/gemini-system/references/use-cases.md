...
- Identify timestamps for key sections" --file "tutorial.mp4" 2>/dev/null

# Code review video
gemini -p "Extract code patterns and best practices demonstrated in this video" --file "code-review.mp4" 2>/dev/null
```
...
- Action items
- Open questions
- Technical terms mentioned" --file "meeting.mp3" 2>/dev/null

# Podcast/talk analysis
gemini -p "Extract technical insights from this talk about {topic}" --file "conference-talk.mp3" 2>/dev/null
```
...
- Rate limiting rules" --file "api-spec.pdf" 2>/dev/null
...
- Edge cases to handle" --file "spec.pdf" 2>/dev/null
...
- Proposed approach
- Key algorithms
- How to apply this in practice" --file "paper.pdf" 2>/dev/null
```
...
gemini -p "Generate comprehensive documentation for src\\auth\" \
  --include-directories src\\auth 2>/dev/null > docs\\auth-module.md
```
